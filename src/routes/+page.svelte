<script lang="ts">
	...
	import type { PageData } from "./$types";	// Cargamos la PageData de +page.ts
	export let data: PageData;					// y la exportamos para usarla en +page.svelte

	let formData: CodeSnippetInput = {	        // Inicializamos los datos del formulario que se actualizará con
		title: "",                              // bind:value en cada input y select del form.
		language: "html",
		code: "",
	};

	snippetStore.set( data.snippets );          // Cargamos el estado de snippets con la data de +page.ts
	...
	<div class="grid grid-cols-1 gap-4 min-w-full md:min-w-[750px]">
		...
		<div class="card p-4 w-full text-token space-y-4">
			<label class="label">
				...
				<input class="input" type="text" placeholder="Enter Title" bind:value={formData.title}/>
			</label>
			<label class="label">
